...
const ProjectManager: React.FC<ProjectManagerProps> = ({ projects, onAddProject, onUpdateProject }) => {
      ...
      <div className="flex overflow-x-auto pb-4 gap-4 no-scrollbar">
        {years.length > 0 ? years.map(year => (
          <button
            ...
            className={`flex-shrink-0 px-8 py-4 rounded-[1.5rem] font-black text-xs transition-all border ${
              selectedYear === year 
                  ...
                  <div className="relative h-4 w-full bg-slate-50 rounded-full overflow-hidden shadow-inner border border-slate-100">
                    <div 
                      className={`absolute top-0 left-0 h-full transition-all duration-1000 ${
                        isOverBudget ? 'bg-red-500' : percentSpent > 80 ? 'bg-amber-500' : 'bg-indigo-600'
                      }`}
                          ...
                          selectedProject.tasks.map((task) => {
                            ...
                            return (
                              <div 
                                id={`task-${task.id}`}
                                key={task.id} 
                                className={`p-8 rounded-[2rem] border transition-all flex items-center justify-between group ${
                                task.completed 
