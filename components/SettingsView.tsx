...
const SettingsView: React.FC<SettingsViewProps> = ({ currentUser, ollamaConfig, setOllamaConfig, isDarkMode, toggleDarkMode, onNavigate }) => {
                     ...
                     <div className="p-8 space-y-4">
                        <div className="flex items-center gap-6">
                           ...
                           <div className="flex-1">
                              <h4 className="font-black text-slate-800 dark:text-white">Ollama Node (Local AI)</h4>
                              <p className="text-[10px] text-slate-400 mt-1 max-w-sm">
                                 Conecta tu NUC o servidor local. Usa Tailscale Serve para obtener una URL HTTPS segura si usas la web desplegada.
                              </p>
                           </div>
                           <div className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest ${ollamaConfig.isActive ? 'bg-emerald-100 text-emerald-600' : 'bg-slate-100 text-slate-400'}`}>
                              {ollamaConfig.isActive ? 'Activo' : 'Inactivo'}
                           </div>
                        </div>

                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pl-[88px]">
                           <div>
                              <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">URL del Nodo (HTTPS recomendado)</label>
                              <input
                                 className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-3 font-bold text-xs text-slate-700 dark:text-white outline-none focus:ring-2 focus:ring-indigo-500/20"
                                 value={ollamaConfig.baseUrl}
                                 onChange={(e) => {
                                    const newConfig = { ...ollamaConfig, baseUrl: e.target.value };
                                    setOllamaConfig(newConfig);
                                    localStorage.setItem('filehub_ollama_config', JSON.stringify(newConfig));
                                 }}
                                 placeholder="https://tu-nodo.ts.net"
                              />
                           </div>
                           <div>
                              <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Modelo LLM</label>
                              <input
                                 className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-3 font-bold text-xs text-slate-700 dark:text-white outline-none focus:ring-2 focus:ring-indigo-500/20"
