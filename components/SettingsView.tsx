...
const SettingsView: React.FC<SettingsViewProps> = ({ currentUser, ollamaConfig, isDarkMode, toggleDarkMode }) => {
        ...
        <div className="lg:col-span-8 space-y-10">
           ...
           <section className="bg-white dark:bg-slate-900 rounded-[3rem] border border-slate-100 dark:border-slate-800 shadow-sm overflow-hidden">
              ...
              <div className="divide-y divide-slate-50 dark:divide-slate-800">
                 {[
                   { name: 'Google Calendar', icon: Globe, status: 'Conectado', color: 'text-blue-500' },
                   { name: 'Ollama Node (Local)', icon: Server, status: ollamaConfig.isActive ? 'Activo' : 'Offline', color: 'text-cyan-500' }
                 ].map((app, i) => (
                   <div key={i} className="p-8 flex items-center justify-between hover:bg-slate-50/50 dark:hover:bg-slate-800/50 transition-all group">
                      ...
                      <button className="px-6 py-2.5 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-[10px] font-black uppercase tracking-widest text-slate-400 hover:text-indigo-600 hover:border-indigo-600 transition-all">
                         Gestionar
                      </button>
                   </div>
                 ))}
