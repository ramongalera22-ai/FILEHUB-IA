...
const OmniAssistant: React.FC<OmniAssistantProps> = ({ globalContext, onAddExpenses, ollamaConfig }) => {
  ...
  const handleSend = async () => {
    ...
    try {
      if (mode === 'chat') {
        ...
        if (useLocal && ollamaConfig && ollamaConfig.isActive) {
          const ollamaResponse = await chatWithOllama(finalPrompt, ollamaConfig, globalContext);

          // Detect financial data injection (basic JSON extraction for consistency)
          ...
          if (jsonMatch && onAddExpenses) {
            try {
              ...
              console.error("Error parsing JSON from Ollama chat", e);
            }
          }
          ...
          <div className="flex overflow-x-auto p-3 bg-slate-50 border-b border-slate-100 gap-2 custom-scrollbar no-scrollbar">
            {[
              ...
              <button
                ...
                className={`flex-shrink-0 flex items-center gap-3 px-5 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all ${mode === m.id ? 'bg-white shadow-md text-indigo-600 border border-indigo-100' : 'text-slate-400 hover:bg-white/50'
                  }`}
              >
          ...
          <div className="flex-1 overflow-y-auto p-8 space-y-8 custom-scrollbar bg-slate-50/30">
            ...
            {messages.map((m, i) => (
              <div key={i} className={`flex flex-col ${m.role === 'user' ? 'items-end' : 'items-start'}`}>
                <div className={`max-w-[90%] rounded-[2rem] p-6 shadow-sm relative group transition-all ${m.role === 'user' ? 'bg-indigo-600 text-white' : 'bg-white border border-slate-100 text-slate-800'
                  }`}>
          ...
          {mode === 'chat' && (
            <div className="px-8 py-3 bg-slate-50 border-t border-slate-100 flex gap-4 overflow-x-auto no-scrollbar">
              ...
              <button
                ...
                className={`flex items-center gap-2 px-4 py-2 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all whitespace-nowrap ${useLocal ? 'bg-orange-100 text-orange-700 border border-orange-200 shadow-sm' : 'text-slate-400 bg-white border border-slate-100'}`}
              >
                <Database size={14} /> Local NUC
              </button>
            </div>
          )}
