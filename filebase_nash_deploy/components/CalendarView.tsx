                ...
                <div className="flex-1 grid grid-cols-7 gap-px bg-slate-100 border border-slate-100 rounded-[2.5rem] overflow-hidden">
                  ...
                  {Array.from({ length: daysInMonth(currentDate) }).map((_, i) => {
                    ...
                    return (
                      ...
                      >
                        <span className={`text-sm font-black ${isToday ? 'bg-indigo-600 text-white w-7 h-7 flex items-center justify-center rounded-xl' : isSelected ? 'text-indigo-600' : 'text-slate-400'
                          }`}>
                          {day}
                        </span>
                        <div className="mt-2 space-y-1">
                          {dayEvents.slice(0, 3).map((event, idx) => (
                            <div key={idx} className={`h-1 rounded-full ${event.type === 'expense' ? 'bg-emerald-400' :
                  ...
                  {eventsForSelectedDay.length === 0 ? (
                    ...
                    eventsForSelectedDay.map((ev, i) => (
                      <div key={i} className="flex gap-8 group">
                        ...
                        <div className={`flex-1 p-8 rounded-[2rem] border transition-all flex items-center justify-between ${ev.type === 'project' ? 'bg-amber-50 border-amber-100 text-amber-900' :
