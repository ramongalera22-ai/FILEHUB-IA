...
import { OllamaConfig, OpenNotebookConfig, AnythingLLMConfig, ChatMessage } from '../types';
import {
  Cpu,
  Globe,
  Settings,
...
const AIHubView: React.FC<AIHubViewProps> = ({
  ...
  const [mode, setMode] = useState<'cloud' | 'local' | 'notebook' | 'anything'>('cloud');
              ...
              {messages.length === 0 && (
                <div className="h-full flex flex-col items-center justify-center text-center space-y-6 opacity-40">
                  ...
                  <p className="font-black text-xs uppercase tracking-[0.3em] text-slate-500 dark:text-slate-400">
                    Nodo {mode === 'cloud' ? 'Google Cloud' : mode === 'local' ? 'Ollama Local' : mode === 'anything' ? 'AnythingLLM' : 'OpenNotebook'} listo
                  </p>
                </div>
              )}
