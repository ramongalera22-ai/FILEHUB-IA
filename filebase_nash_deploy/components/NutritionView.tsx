  ...
  const handlePlanUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      try {
        ...
        alert(`Error al subir el plan: ${error.message}`);
      } finally {
            ...
            <div className="grid grid-cols-1 md:grid-cols-7 gap-4">
              {weeklyPlan.map((plan, idx) => (
                <button
                  ...
                  className={`p-4 rounded-2xl border transition-all text-left group h-full flex flex-col ${selectedDayIndex === idx
            ...
            <div className="flex justify-between items-center mb-10">
              ...
              <div className="flex gap-3">
                ...
                <button
                  ...
                  className="bg-indigo-600 text-white px-6 py-3 rounded-2xl font-black text-[10px] uppercase tracking-widest flex items-center gap-2 hover:bg-indigo-700 transition-all shadow-xl shadow-indigo-200 disabled:opacity-50"
                >
                  {isUploadingPlan ? <Loader2 size={16} className="animate-spin" /> : <FileUp size={16} />}
                  {isUploadingPlan ? 'Subiendo...' : 'Subir Plan'}
                </button>
              </div>
            </div>
